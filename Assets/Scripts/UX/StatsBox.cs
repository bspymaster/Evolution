...
public class StatsBox : TileData
{
    ...
    public void OnMouseDown()
    {
        if (toggle == false)
        {
            ...
            foods.transform.SetParent(GameObject.FindGameObjectWithTag("Canvas").transform, false);

            Text biomeTitle = Instantiate(biomePrefab, new Vector3(0, 125, -5), Quaternion.identity) as Text;
            biomeTitle.transform.SetParent(GameObject.FindGameObjectWithTag("Canvas").transform, false);
